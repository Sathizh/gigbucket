<x-layout>
    <x-card class=" max-w-lg mx-auto mt-24">
        ...
        <form action="/listings/{{$listing->id}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                <label class="block">
                    <span class="inline-block text-lg mb-2">Company logo</span>
                  ...
                  @error('logo')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
                <img class="w-48 mr-6 mb-6 m-2" src="{{ $listing->logo ? asset('storage/'.$listing->logo) : asset('images/no-image.png') }}" alt='{{$listing->company}}' />
            </div>
